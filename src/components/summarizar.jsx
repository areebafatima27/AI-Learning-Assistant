...
import { Upload, File, FileText, X, Sparkles, CheckCircle, Bell } from 'lucide-react';
...
export default function Summarize() {
  ...
  const handleSummarize = async () => {
    if (!uploadedFile) {
      setSummaryResult("No file uploaded.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        if (!data.summary?.trim()) {
          setSummaryResult(
            "The file appears to have no extractable text to summarize."
          );
        } else {
        ...
        <div className="fixed top-6 right-6 z-50" style={slideInStyle}>
          <div className="bg-white/95 backdrop-blur-lg border border-emerald-200 rounded-xl shadow-xl shadow-emerald-100/50 p-4 flex items-center space-x-3 max-w-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-600">Your document has been successfully summarized.</p>
            </div>
            <button
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-8 relative z-10">
        ...
        <div className="text-center space-y-4" style={fadeUpStyle}>
          ...
          <p className="text-gray-600 text-lg font-medium max-w-md mx-auto">
            Transform lengthy documents into concise, intelligent summaries in seconds
          </p>
        </div>
        ...
        {/* Upload Progress */}
        {isUploading && (
          <Card className="border border-blue-200 bg-blue-50/80 backdrop-blur-sm shadow-lg" style={fadeUpStyle}>
            <CardContent className="p-12">
              <div className="text-center space-y-6">
                <div className="flex justify-center">
                  <div className="p-6 bg-blue-100 rounded-2xl shadow-lg">
                    <Upload className="h-12 w-12 text-blue-600" style={spinElegantStyle} />
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="w-full bg-blue-200 rounded-full h-3 overflow-hidden shadow-inner">
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full shadow-sm" style={progressStyle}></div>
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        {error && (
          <div style={shakeStyle}>
            <Alert variant="destructive" className="bg-red-50/80 backdrop-blur-sm border-red-200 shadow-lg" style={fadeUpStyle}>
        ...
        {uploadedFile && (
          <Card className="border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-500 bg-white/80 backdrop-blur-sm" style={scaleInStyle}>
        ...
        {/* Summary Result */}
        {showSummary && (
          <Card className="border border-emerald-200 bg-gradient-to-br from-emerald-50/80 to-green-50/80 backdrop-blur-sm shadow-lg" style={revealUpStyle}>
            <CardContent className="p-8">
              <div className="space-y-6">
                <div className="flex items-center justify-center space-x-3">
                  <div className="p-3 bg-gradient-to-r from-emerald-500 to-green-500 rounded-full shadow-lg">
                    <Sparkles className="h-6 w-6 text-white" style={sparkleStyle} />
                  </div>
                  ...
                  <p className="text-gray-700 leading-relaxed font-medium">
                    {summaryResult || (
                      <span className="flex items-center space-x-2">
                        <div className="rounded-full h-4 w-4 border-2 border-emerald-500 border-t-transparent" style={spinnerStyle}></div>
